...
export default async function handler(req: Request) {
  ...
  try {
    if (req.method === 'PUT') {
      ...
      if (!ok) {
        ...
        return new Response(JSON.stringify({
          ...
          message: 'Cannot overwrite non-empty clipping pipeline with empty data. This action requires manual approval.',
        }), {
          status: 409,
    ...
    if (data === null) {
      return new Response(JSON.stringify({ leads: [], version: 1, updatedAt: new Date().toISOString() }), {
        status: 200,
